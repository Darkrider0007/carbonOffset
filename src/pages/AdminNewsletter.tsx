...
export default function AdminNewsletter() {
    ...
    useEffect(() => {
        const fetchAllNewsletterSubscribers = async () => {
            ...
            try {
                ...
                setDashBoardData({ percentageSubscriberIncrease: res.data.percentageIncrease });
            } catch (error) {
    ...
    const downloadCSV = () => {
        const csvContent = "data:text/csv;charset=utf-8,"
            + newsletterSubscribers.map((subscriber: { email: any; }) => subscriber.email).join("\n");
                                        ...
                                        <>
                                            ...
                                            <h1>
                                                <span className="text-green-600">
                                                    {Math.abs(dashBoardData?.percentageSubscriberIncrease) || 0}
                                                </span>{" "}
                                                % vs last month
                                            </h1>
                                        </>
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {currentSubscribers.length > 0 ? (
                                        currentSubscribers.map((subscriber: any, index: number) => (
