...
const CarbonOffsetCalculator: React.FC = () => {
  ...
  const [selectedPaymentOption, setSelectedPaymentOption] =
    useState<string>(""); // For payment option selection
  ...
  const calculateOffset = (type: string, multiplier: number): OffsetValues => {
    return {
      netNegativeImmediate:
        multiplier * OFFSET_VALUES[type].netNegativeImmediate,
  ...
  const totalRemoteOffset = calculateOffset(
    "remoteWorker",
    Number(allFields.remoteWorkers || 0)
  );
  const totalOnsiteOffset = calculateOffset(
    "onsiteWorker",
    Number(allFields.onsiteWorkers || 0)
  );
  const totalTravelingOffset = calculateOffset(
    "travelingWorker",
    Number(allFields.travelingWorkers || 0)
  );
  const totalEmissionOffset = calculateOffset(
    "additionalEmission",
    Number(allFields.additionalEmissions || 0)
  );
  ...
  const options = [
    ...
    {
      label: "Additional Emissions (per CO2 MT / year)",
      value: "additionalEmissions",
    },
  ];
  ...
  const handleCheckoutNow = async () => {
    try {
      ...
      if (selectedOffset === "ImmediateNetNegative") {
        tokens = totalOffset.netNegativeImmediate;
      } else if (selectedOffset === "Neutral5Years") {
        tokens = totalOffset.neutral5Years;
      } else if (selectedOffset === "Neutral1Year") {
        tokens = totalOffset.neutral1Year;
      } else if (selectedOffset === "NetNegative5Years") {
        tokens = totalOffset.netNegative5Years;
      }
      const token = await getTokenData();
      const tokenPrice = token.data.tokenPrice;

      const cost = tokens * tokenPrice;

      const res = await AddToWallet({
        amount: cost,
        tokens: tokens,
      });
  ...
  const handleCheckoutNowBusiness = async () => {
    try {
      let tokens = 0;

      if (selectedPaymentOption === "Option1") {
        tokens = 250;
      } else if (selectedPaymentOption === "Option2") {
        tokens = 30;
      } else if (selectedPaymentOption === "Option3") {
        tokens = 3;
      }
    ...
    <div className="max-w-4xl mx-auto p-8 bg-gray-100 rounded-lg shadow-lg">
      ...
      <div className="mb-6">
        <p className="text-lg text-gray-700 mb-4">
          If you are an individual, select "Additional Emissions (per CO2 MT /
          year)" and the type of worker you are. For example, if you are a
          remote worker, select that option and input "1". If you run a
          business, select the relevant worker categories and add their
          quantities.To calculate the additional emission you can refer the{" "}
          <span
            onClick={() => navigate("/calculator/calculationMethods")}
        ...
        <Accordion type="single" collapsible className="w-full">
          <AccordionItem value="item-1">
            ...
            <AccordionContent>
              If you are working remotely, your carbon footprint is generally
              lower due to the absence of daily commuting. By staying at home,
              you reduce emissions from transportation and limit the energy
              consumption needed to power large office spaces. Remote workers
              contribute to sustainability by lowering the need for heating,
              cooling, lighting, and other office infrastructure. If you’re
              working from home, please select this option to calculate your
              reduced carbon impact.
            </AccordionContent>
          </AccordionItem>
          <AccordionItem value="item-2">
            <AccordionTrigger>
              In-Office Worker Carbon Footprint
            </AccordionTrigger>
            <AccordionContent>
              If you work from an office, your carbon footprint will likely be
              higher due to daily commutes and the energy consumed by shared
              workspaces. Office environments require significant energy for
              climate control, equipment usage, and amenities, in addition to
              the emissions generated from transportation. If you’re primarily
              commuting to a workplace, please choose this option to accurately
              reflect your environmental impact.
            </AccordionContent>
          </AccordionItem>
        </Accordion>
      </div>
        ...
        <>
          ...
          >
              ...
              <div className="flex flex-col">
                ...
                <input
                  ...
                  className={`p-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 ${
                    errors.remoteWorkers ? "border-red-500" : ""
                  }`}
                  disabled={showResults}
                />
                {errors.remoteWorkers && (
            ...
            {selectedOptions.includes("onsiteWorkers") && (
              <div className="flex flex-col">
                ...
                <input
                  ...
                  className={`p-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 ${
                    errors.onsiteWorkers ? "border-red-500" : ""
                  }`}
                  disabled={showResults}
                />
                {errors.onsiteWorkers && (
                  <p className="text-red-500 text-sm">This field is required</p>
                )}
              </div>
            )}
            ...
            {selectedOptions.includes("travelingWorkers") && (
              <div className="flex flex-col">
                ...
                <input
                  ...
                  className={`p-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 ${
                    errors.travelingWorkers ? "border-red-500" : ""
                  }`}
                  disabled={showResults}
                />
                {errors.travelingWorkers && (
                  <p className="text-red-500 text-sm">This field is required</p>
                )}
              </div>
            )}
            ...
            {selectedOptions.includes("additionalEmissions") && (
              <div className="flex flex-col">
                <label className="text-lg font-semibold mb-2">
                  Additional Emissions (per CO2 MT / year):
                </label>
                <input
                  type="number"
                  min="0"
                  ...
                  className={`p-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 ${
                    errors.additionalEmissions ? "border-red-500" : ""
                  }`}
                  disabled={showResults}
                />
                {errors.additionalEmissions && (
                  <p className="text-red-500 text-sm">This field is required</p>
                )}
              </div>
            )}
      ...
      {showResults && (
        <div className="bg-white p-6 rounded-lg shadow-md mb-8">
            ...
            <div className="flex flex-col space-y-2">
              <label>
                <input
                  ...
                  className="m-2"
                />
                Immediate Net Negative:{" "}
                {totalOffset.netNegativeImmediate.toFixed(2)}
              </label>
              <label>
                <input
                  type="radio"
              ...
              <label>
                <input
                  ...
                  className="m-2"
                />
                Net Negative in 5 Years:{" "}
                {totalOffset.netNegative5Years.toFixed(2)}
              </label>
            </div>
            <div className="w-full py-4 ">
              {selectedOffset && (
                <Button
                  className="bg-green-600 hover:bg-green-700 flex flex-row items-center justify-center gap-2"
                  onClick={handleCheckoutNow}
                >
                  <TreePineIcon /> Offset Now
                </Button>
              )}
            </div>
          </div>
          ...
          <div className="mb-4">
            <label className="text-lg font-semibold mb-2">
              Select Payment Option: (For businesses)
            </label>
            <div className="flex flex-col space-y-2">
              <label>
                <input
                  ...
                  className="m-2"
                />
                Option 1: A one-time, all-inclusive investment of 250 UNY /
                offset unit that covers you and your business for the life of
                your operations
              </label>
              <label>
                <input
                  ...
                  className="m-2"
                />
                Option 2: A yearly recurring cost of 30 UNY / offset unit that
                covers you and your business for the year of your operations
              </label>
              <label>
                <input
                  type="radio"
                  ...
                  className="m-2"
                />
                Option 3: A monthly recurring cost of 3 UNY / offset unit that
                covers you and your business for each month of your operations
              </label>
            </div>
            {selectedPaymentOption && (
              <Button
                className="bg-green-600 hover:bg-green-700 flex flex-row items-center justify-center gap-2"
                onClick={handleCheckoutNowBusiness}
              >
                <TreePineIcon /> Offset Now
              </Button>
            )}
