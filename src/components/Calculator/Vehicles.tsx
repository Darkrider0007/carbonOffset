...
function Vehicle({ addInput }: VehicleProps) {
  ...
  const defaultVehicles = savedVehicles
    ? JSON.parse(savedVehicles)
    : [{ milesPerYear: "", fuelEfficiency: "" }];

  const savedNumVehicles = localStorage.getItem("numVehicles");
  const defaultNumVehicles = savedNumVehicles
    ? parseInt(savedNumVehicles, 10)
    : 1;
  ...
  const [customVehicleCount, setCustomVehicleCount] = useState<number | null>(
    null
  );
  ...
  const handleCustomVehicleCountChange = (
